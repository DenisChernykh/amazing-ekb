        ...
        <Input
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === "ArrowDown") {
              e.preventDefault();
              setActiveIndex((prev) =>
                prev < telegramPosts.length - 1 ? prev + 1 : 0,
              );
            }
            ...
            if (e.key === "ArrowUp") {
              e.preventDefault();
              setActiveIndex((prev) =>
                prev > 0 ? prev - 1 : telegramPosts.length - 1,
              );
            }

            if (e.key === "Enter" && activeIndex !== -1) {
              e.preventDefault();
              const selected = telegramPosts[activeIndex];
              if (selected) {
      ...
      {isOpen && (
        <div className="absolute top-full z-10 mt-1 w-full rounded-md border border-gray-200 bg-white shadow-lg">
          <div className="max-h-60 overflow-auto p-1">
            {telegramPosts.map((tgPost, index) => {
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    setValue(
                      "tgImages",
                      tgPost.images
                        .map((img) => img.imageUrl) // ["url1.jpg", "url2.jpg", ...]
                        .filter((url): url is string => url !== null), // убираем null
                    );
                    setIsOpen(false);
                  }}
                >
